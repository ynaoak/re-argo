...
//! 4. Runs the analysis pipeline (`reargo_program::Program` + the full
//!    `AnalysisManager`).
...
fn analyze(path: &Path) -> Option<reargo_program::Program> {
    let mut program = reargo_program::Program::from_binary(path).ok()?;
