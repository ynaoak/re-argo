...
//! `load_config` field. (Our `reargo-loader::pe` doesn't yet, so the x86
...
impl Analyzer for PeSehAnalyzer {
    ...
    fn analyze(&self, program: &mut Program) -> Result<AnalysisResult, AnalysisError> {
        if program.info.format != reargo_loader::BinaryFormat::Pe {
...
fn parse_tls_callbacks(program: &mut Program, base: u64, size: u64) -> usize {
    ...
    for chunk in buf.chunks_exact(ptr_size) {
        ...
        if !program.listing.has_function(ptr) {
            program.listing.add_function(reargo_program::function::Function::new(
                ptr,
        ...
        program.symbol_table.add(reargo_program::symbol::Symbol::new(
            format!("tls_callback_{}", idx),
            ptr,
            reargo_program::symbol::SymbolType::Function,
            reargo_program::symbol::SourceType::Analysis,
        ));
        if program
            .comments
            .get(ptr, reargo_program::comments::CommentType::Plate)
            .is_none()
        {
            program.comments.set(
                ptr,
                reargo_program::comments::CommentType::Plate,
