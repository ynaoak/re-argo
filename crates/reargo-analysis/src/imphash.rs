...
pub fn compute_pe_imphash(imports: &[reargo_loader::ImportEntry]) -> Option<String> {
    if imports.is_empty() {
        return None;
    }
    let mut pairs: Vec<String> = Vec::with_capacity(imports.len());
    for imp in imports {
        // reargo-loader's ImportEntry doesn't carry a library name today
...
pub fn compute_elf_symhash(imports: &[reargo_loader::ImportEntry]) -> Option<String> {
    if imports.is_empty() {
        return None;
    }
...
mod tests {
    ...
    fn imp(name: &str) -> reargo_loader::ImportEntry {
        reargo_loader::ImportEntry {
            name: name.to_string(),
            plt_address: 0,
            got_address: 0,
        }
